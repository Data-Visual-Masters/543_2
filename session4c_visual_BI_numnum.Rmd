...
A particular period:
```{r, eval=TRUE}
# just changing view, not the data
...
We could re organize the previous plot so that we highlight the most and least common crimes along that time period. However, as you have seen, we have not created a frequency table of dates, we have just used lubridate's _floor_date_.   Then, let me create a frequency table:
```{r, eval=TRUE}
...
Now, we can use the _count_ column in our new _crimeDate_ data frame:

```{r, eval=TRUE}
base=ggplot(crimeDate[crimeDate$date>'2014-12-31',], # filter
            aes(x=date,y=count))
barT4=base + geom_bar(stat = 'identity') 
barT4 + facet_wrap(~reorder(crime,-count)) # descending
```
...
Remember that although the boxplot is very informative, it is advisable to  try familiar  statistics :
...
Besides conditionally annotating the places, you can identify the area where of the most salient behavior. Let's highlight overlaping of points:
```{r hexbins, eval=TRUE}
