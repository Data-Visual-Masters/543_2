...
densePlot=densePlot+ facet_wrap(~year(crimeTotalCountsDay$date),
...
heat2 = base +  stat_density_2d(aes(fill = ..density..), 
                                 geom = "raster", contour = FALSE) + scale_fill_distiller(palette="Greys", direction=1) 
heat2
```
...
```{r, eval=TRUE}
heat2 +  scale_x_continuous(expand = c(0, 0)) + 
...
```{r, eval=TRUE}
base = ggplot(num_num, aes(x=DaysToReport,y=CrimeShare)) 
heatZoom= base +  geom_point() + xlim(c(2,5))+ ylim(c(0,3))
heatZoom= heatZoom + geom_text_repel(aes(label=Neighborhood),size=2)
heatZoom
```
